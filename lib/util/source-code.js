...
function SourceCode(text, ast) {
    ...
    /*
     * Shebangs are parsed as regular "Line" comments. Changing the type allows
     * for rules to have more fine-grained control.
     */
    const shebangCheck = this.text.match(/^#!([^\r\n]+)/);
    const hasShebang = shebangCheck && ast.comments.length && ast.comments[0].value === shebangCheck[1];

    if (hasShebang) {
        ast.comments[0].type = "Shebang";
    }

    // The shebang should be included in tokensAndComments but not in the top level comments array.
